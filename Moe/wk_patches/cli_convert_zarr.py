...
def _zarr_chunk_converter(
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "ZSTD_decompressStream" in str(e) and attempt < max_retries - 1:
                # Extract corrupted file path from error and drop OS page cache + TS cache
                import os, re
                match = re.search(r'"([^"]+)"', str(e))
                if match:
                    corrupt_path = match.group(1)
                    try:
                        fd = os.open(corrupt_path, os.O_RDONLY)
                        os.posix_fadvise(fd, 0, 0, os.POSIX_FADV_DONTNEED)
                        os.close(fd)
                    except OSError:
                        pass
                # Clear TensorStore's in-process cache by forcing GC
                del zarr_file
                import gc
                gc.collect()
                logger.warning(
                    "Transient NFS read error (attempt %d/%d) for %s: %s. Dropped page cache, retrying in 10s...",
                    attempt + 1, max_retries, bounding_box.topleft, e,
                )
                time.sleep(10)
            else:
                raise
