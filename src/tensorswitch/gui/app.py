...
try:
    from tensorswitch.gui.lab_paths_system import get_all_projects
    from tensorswitch.gui.lab_paths_system.path_selector import create_simple_path_helper_panel
...
try:
    from tensorswitch.gui.format_detection import FormatDetector, TaskPlanner
...
try:
    from tensorswitch.gui.ai import create_floating_ai_assistant, ai_config
...
try:
    from tensorswitch.gui.cost_estimator import (
...
class SimpleTensorSwitchGUI(param.Parameterized):
    ...
    memory_limit = param.Integer(
        default=50,
        bounds=(10, 90),
        doc="Memory limit percentage (only applies to 'Run Local' mode; cluster jobs use LSF memory allocation)"
    )
...
Click **🚀 Run Job** to execute this configuration.
    ...
    def _generate_coordinator_script(self, input_path, output_path, max_level):
        ...
        # Get commands to run tensorswitch
        # TODO: Full apptainer integration requires modifying job submission in __main__.py
        # For now, use native Python installation which works correctly

        # Use native Python for both submission and workers
        ...
        # Apptainer support - currently disabled pending full integration
        # Will re-enable after coordinating with Goran on proper container setup
        use_apptainer = False

        # Future: When apptainer is fully integrated, worker jobs will use:
        # apptainer exec --bind /groups:/groups --bind /nrs:/nrs --bind /scratch:/scratch
        #   /misc/local/singularity/tensorswitch-gui.sif python -m tensorswitch
...
# Deployment: {'Apptainer Container' if use_apptainer else 'Local Development'}
...
echo "Deployment: {'Apptainer' if use_apptainer else 'Local'}"
echo "Started: $(date)"
echo "========================================="
...
submit_and_wait() {{
    ...
    # Use submit_cmd (native Python) so bsub works
    output=$({submit_cmd} \\
        --task $task \\
        --base_path "$base_path" \\
        --output_path "$output_path" \\
...
"""
        ...
        if max_level > 0 and downsample_task:
            for level in range(1, max_level + 1):
                # Use new OME-Zarr compliant structure (s0, s1, s2... at root, no multiscale/ folder)
                prev_level_path = os.path.join(output_path, f"s{level-1}")
